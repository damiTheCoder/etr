...
export default function SalesInvoiceForm() {
  ...
  const [lineItems, setLineItems] = useState<any[]>([
    { item: '', quantity: 1, rate: 0, amount: 0, account: 'Sales' }
  ])
  ...
  useEffect(() => {
    async function loadMasters() {
      try {
        ...
        const filteredParties = (p as any[]).filter(x => x.partyType !== 'Supplier')
        setParties(filteredParties.length > 0 ? filteredParties : (p as any[]))
        setItems(i as any[])
        setTaxes(t as any[])
      } catch (e) {
        console.error(e)
  ...
  function addItem() {
    setLineItems([...lineItems, { item: '', quantity: 1, rate: 0, amount: 0, account: 'Sales' }])
  }
  ...
  function handleItemNameChange(index: number, val: string) {
    ...
    if (selected) {
      ...
      updated[index].account = selected.incomeAccount || 'Sales'
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      const taxRows = taxRate > 0 ? [{
        tax: selectedTax === 'custom' ? `Custom Tax (${taxRate}%)` : (selectedTax || 'Tax'),
        account: selectedTaxObj?.account || 'Output Tax Payable',
        rate: taxRate,
        amount: taxAmount,
      }] : []
      ...
      const res = await api.create('SalesInvoice', data)
      await api.submit('SalesInvoice', res.name)
  ...
  return (
    <div>
      <div className="flex items-center gap-3 sm:gap-4 mb-6">
        <Link to="/sales-invoices" className="shrink-0 text-gray-400 hover:text-gray-600">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">{isEdit ? 'Edit Sales Invoice' : 'New Sales Invoice'}</h1>
      </div>
